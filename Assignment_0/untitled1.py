...
Original file is located at
    https://colab.research.google.com/drive/1j0ET6lNbsOWbckhMUmTSxu6qn9_6EX3D
"""
...
def train_tree(data,y, target_factor, max_depth = None,min_samples_split = None, min_information_gain = 1e-20, counter=0, max_categories = 1500):
  '''
...
# Read the train data
train_filename = "census+income+kdd (1)\census.tar\census\census-income.data"
train_data = pd.read_csv(train_filename,  skipinitialspace=True, header = None)
...
# Read the train data
test_filename = "census+income+kdd (1)\census.tar\census\census-income.test"
test_data = pd.read_csv(test_filename,  skipinitialspace=True, header = None)
test_data.columns = ["age", "class of worker", "detailed industry recode", "detailed occupation recode", "education", "wage per hour", "enroll in edu inst last wk", "marital stat", "major industry code", "major occupation code", "race", "hispanic origin", "sex", "member of a labor union", "reason for unemployment", "full or part time employment stat", "capital gains", "capital losses", "dividends from stocks", "tax filer stat", "region of previous residence", "state of previous residence", "detailed household and family stat", "detailed household summary in household", "instance weight", "migration code-change in msa", "migration code-change in reg", "migration code-move within reg", "live in this house 1 year ago", "migration prev res in sunbelt", "num persons worked for employer", "family members under 18", "country of birth father", "country of birth mother", "country of birth self", "citizenship", "own business or self employed", "fill inc questionnaire for veteran's admin", "veterans benefits", "weeks worked in year", "year", "income"]

import numpy as np

train_data1 = train_data.drop(columns=["instance weight"])
titanic_lite = train_data1[0:100]
titanic_lite = titanic_lite.loc[titanic_lite.isna().sum(axis = 1)==0,:]
titanic_tree = train_tree(titanic_lite,'income',True, max_depth=5, min_samples_split=10)

titanic_tree

titanic_prediction = []
num_obs = len(titanic_lite)
max_depth = 5
...
accuracy = np.sum(titanic_prediction == titanic_lite.Survived[:num_obs].to_numpy()) / len(titanic_prediction)
print(f"Max Depth: {max_depth}, Training Accuracy: {accuracy:.4f}")